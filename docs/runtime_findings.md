...
The [curated real evidence](../evidence/real_runs/20260910T144212150699Z_baseline_e2e/README.md) includes environment/version/capability/commit metadata, current logs, parser output, state, traffic, scenario results, and bounded readiness results. Earlier attempt failures and baseline teardown/endpoint warnings remain disclosed. That September 10 run did not establish fault/recovery, throughput, IPv6 session, packet-capture, RF, or other-host success. The later evidence/documentation commit is not the runtime-executing commit. The VM lab was left running; PR #11 was not yet merged at that time.

## September 13 audit and merged status

[PR #11](https://github.com/omidrahimirad/open5gs-ueransim-5g-sa-lab/pull/11) is now merged into main at `20caaf8dd25de284043c1ba483f92a37514ad4da`. The latest runtime-executed commit remains `daca4542608f0bb20ca01441f5e584ce89890ad1`, distinct from the merge and evidence-only commits.

The [September 13 audit evidence](../evidence/real_runs/20260913_pre_merge_audit/README.md) records a fresh baseline PASS (25/25), a mandatory final health gate after 5/5 DN replies, and healthy core/RAN with zero restarts. Real N3 impairment caused 100% loss through a scoped UDP/2152 drop; verified rollback restored 5/5 replies and passed all 11 recovery checks. The negative audit deliberately failed current evidence collection and correctly returned ERROR without accepting historical `USER_PLANE_FAILURE` evidence.

The earlier findings above retain their historical scope. Other fault scenarios have not received fresh Linux runtime validation. No throughput, IPv6 PDU-session, RF/OTA, PCAP, multi-host/multi-UE, full fault-matrix, carrier-grade, production-readiness, or long-duration reliability claim follows from these runs.
